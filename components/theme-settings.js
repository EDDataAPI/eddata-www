...
const ThemeSettings = () => {
  ...
  const saveChanges = () => {
    ...
    const contrast = convertPercentageToRange(
      100 - l,
      MIN_CONTRAST,
      MAX_CONTRAST
    )
  ...
  const resetToDefaults = () => {
    ...
    const defaultSaturation = window
      .getComputedStyle(document.documentElement)
      .getPropertyValue('--default-color-primary-saturation')
    const defaultContrast = window
      .getComputedStyle(document.documentElement)
      .getPropertyValue('--default-contrast')
    const defaultHilightHueShift = window
      .getComputedStyle(document.documentElement)
      .getPropertyValue('--default-highlight-hue-shift')
    document.documentElement.style.setProperty(
      '--color-primary-hue',
      defaultHue
    )
    ...
    document.documentElement.style.setProperty(
      '--highlight-hue-shift',
      defaultHilightHueShift
    )
    window.localStorage.removeItem('theme-settings')
    const lightness =
      100 -
      convertNumberToPercentage(defaultContrast, MIN_CONTRAST, MAX_CONTRAST)
    setColor(hsl2hex(defaultHue, defaultSaturation.replace('%', ''), lightness))
    setHighlightHueShift(defaultHilightHueShift)
    document.getElementById('highlight-color-hue-shift').value =
      defaultHilightHueShift
  }

  return (
    <div
      style={{ minWidth: '210px', minHeight: '10rem', paddingLeft: '.5rem' }}
    >
        ...
        {Array.from({ length: 11 }, (_, i) => i).map(number => (
          <div
            ...
            style={{
              aspectRatio: '1',
              width: '9%',
              background: `var(--color-highlight-${number})`,
              display: 'inline-block'
            }}
          />
        ))}
      ...
      <div style={{ width: 'calc(100% - .5rem)' }}>
        <input
          style={{ width: '99%' }}
          type='range'
          id='highlight-color-hue-shift'
          name='highlight-color-hue-shift'
          min='0'
          max='360'
          onChange={onChangeHighlightHueShift}
        />
      </div>
      <p className='text-center' onClick={resetToDefaults}>
        RESET
      </p>
    </div>
  )
