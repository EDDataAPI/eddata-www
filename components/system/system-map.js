...
const SystemMap = ({
  ...
  const [mapZoomMultiplier, setMapZoomMultiplier] = useState(
    SYSTEM_MAP_DEFAULT_ZOOM_MULTIPLIER
  )
  ...
  return (
    <div className='fx__fade-in' style={{ position: 'relative' }}>
      <div className='system-map__system-info'>
        ...
        <div className='system-map__system-objects'>
          ...
          {bodiesInSystem !== undefined &&
            ...
            fleetCarriersInSystem !== undefined && (
              <>
                {bodiesInSystem?.length === 0 &&
                  stationsInSystem?.length === 0 && (
                    <p
                      className='fx__fade-in text-blink-slow'
                      style={{ height: '1.75rem' }}
                    >
                      <i
                        className='icon icarus-terminal-warning muted'
                        style={{
                          position: 'relative',
                          top: '.1rem',
                          fontSize: '1.25rem'
                      ...
                      />
                      <small>System not scanned</small>
                    </p>
                  )}
                {bodiesInSystem?.filter(b => b.bodyType === 'Star')?.length >
                  0 && (
                ...
                {bodiesInSystem?.filter(b => b.bodyType === 'Planet')?.length >
                  0 && (
                  <p className='fx__fade-in'>
                    <i className='icon icarus-terminal-planet' />{' '}
                    {
                      bodiesInSystem?.filter(b => b.bodyType === 'Planet')
                        ?.length
                    }
                  </p>
                )}
                {stationsInSystem?.filter(s => s.stationType === 'Orbis')
                  ?.length > 0 && (
                  <p className='fx__fade-in'>
                    <i className='icon icarus-terminal-orbis-starport' />{' '}
                    {
                      stationsInSystem?.filter(s => s.stationType === 'Orbis')
                        ?.length
                    }
                ...
                {stationsInSystem?.filter(s => s.stationType === 'Coriolis')
                  ?.length > 0 && (
                  <p className='fx__fade-in'>
                    <i className='icon icarus-terminal-coriolis-starport' />{' '}
                    {
                      stationsInSystem?.filter(
                        s => s.stationType === 'Coriolis'
                      )?.length
                    }
                  </p>
                )}
                {stationsInSystem?.filter(s => s.stationType === 'Ocellus')
                  ?.length > 0 && (
                  <p className='fx__fade-in'>
                    <i className='icon icarus-terminal-ocellus-starport' />{' '}
                    {
                      stationsInSystem?.filter(s => s.stationType === 'Ocellus')
                        ?.length
                    }
                ...
                {stationsInSystem?.filter(s => s.stationType === 'AsteroidBase')
                  ?.length > 0 && (
                  <p className='fx__fade-in'>
                    <i className='icon icarus-terminal-asteroid-base' />{' '}
                    {
                      stationsInSystem?.filter(
                        s => s.stationType === 'AsteroidBase'
                      )?.length
                    }
                  </p>
                )}
                {stationsInSystem?.filter(s => s.stationType === 'Outpost')
                  ?.length > 0 && (
                  <p className='fx__fade-in'>
                ...
                {stationsInSystem?.filter(
                  s =>
                    s.stationType === 'CraterPort' ||
                    s.stationType === 'CraterOutpost'
                )?.length > 0 && (
                  <p className='fx__fade-in'>
                ...
                {settlementsInSystem?.length > 0 && (
                  <p className='fx__fade-in'>
                    ...
                    {settlementsInSystem.length}
                  </p>
                )}
                {megashipsInSystem?.length > 0 && (
                  <p className='fx__fade-in'>
                    <i className='icon icarus-terminal-megaship' />{' '}
                    {megashipsInSystem.length}
                  </p>
                )}
                {fleetCarriersInSystem?.length > 0 && (
                  <p className='fx__fade-in'>
                    <i className='icon icarus-terminal-fleet-carrier' />{' '}
                    {fleetCarriersInSystem.length}
                  </p>
                )}
                {stationsInSystem?.filter(
                  s =>
                    s.stationType === 'PlanetaryConstructionDepot' ||
                    s.stationType === 'SpaceConstructionDepot' ||
                    s.stationType === 'SurfaceStation'
                )?.length > 0 && (
                  <p className='fx__fade-in'>
                    <i className='icon icarus-terminal-poi-empty' />{' '}
                    {
                      stationsInSystem.filter(
                        s =>
                          s.stationType === 'PlanetaryConstructionDepot' ||
          ...
          <div className='system-map__location'>
            <p>
              <small className='fx__animated-text' data-fx-order='1'>
                ...
                <CopyOnClick>{system.systemAddress}</CopyOnClick>
              </small>
              <br />
              <small
                className='fx__animated-text'
                data-fx-order='1'
                style={{ paddingRight: '1rem' }}
              >
                <span className='muted'>SYS POS</span>{' '}
                <CopyOnClick>
              ...
              {system.tradeZoneLocation !== undefined && (
                <>
                  <br />
                  <small
                    className='fx__animated-text text-no-transform'
                    data-fx-order='3'
                  >
        ...
        {systemStatus !== undefined && systemStatus !== null && (
          <div className='system-map__system-status'>
            ...
            {systemStatus.economy &&
              systemStatus.economy.primary &&
              systemStatus.economy.primary !== 'None' && (
                <span
                  style={{ display: 'block' }}
                  className='fx__animated-text'
                  data-fx-order='6'
                >
                  ...
                  {systemStatus.economy.primary}
                  {systemStatus.economy.secondary &&
                    systemStatus.economy.secondary !== 'None' && (
                      <> &amp; {systemStatus.economy.secondary}</>
                    )}{' '}
                  Economy
                </span>
            ...
            {systemStatus.state && systemStatus.state !== 'None' && (
              ...
              >
                ...
                {factionStates?.[
                  systemStatus.state.replaceAll(' ', '').toLowerCase()
      ...
      <div className='system-map'>
        <div
          className='system-map__point system-map__point--highlighted'
          style={{ top: '50%', left: '50%' }}
          data-name={system.systemName}
        />
        {/* {system && nearbySystems === undefined && <div className='system-map__scanner'/>} */}
        <div
          className={`system-map__scanner${!system || nearbySystems !== undefined ? '--stopped' : ''}`}
        />
        {starsVisible &&
          nearbySystems &&
          nearbySystems.map((nearbySystem, i) => (
            <div
              ...
              onClick={() =>
                router.push(`/system/${nearbySystem.systemAddress}`)
              }
              ...
              style={{
                ...
                top:
                  nearbySystem.systemZ > system.systemZ
                    ? `calc(50% + ${(nearbySystem.systemZ - system.systemZ) * mapZoomMultiplier}px)`
                    : `calc(50% - ${(system.systemZ - nearbySystem.systemZ) * mapZoomMultiplier}px)`, // Z
                left:
                  nearbySystem.systemX > system.systemX
                    ? `calc(50% + ${(nearbySystem.systemX - system.systemX) * mapZoomMultiplier}px)`
                    : `calc(50% - ${(system.systemX - nearbySystem.systemX) * mapZoomMultiplier}px)` // X
              }}
            />
          ))}
      </div>
