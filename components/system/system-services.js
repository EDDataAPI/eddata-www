    ...
    <div className='fx__fade-in'>
      ...
      {nearestServices === undefined && (
        <p
          ...
          style={{ position: 'relative', top: '-.25rem', height: '1.75rem' }}
        />
      )}
      {nearestServices && (
        <div className='fx__fade-in nearby-services'>
          <p>
            Station services in and nearest to{' '}
                      ...
                      <tr>
                        <td style={{ paddingBottom: '1rem', paddingTop: 0 }}>
                          {nearestServices[service]
                            ?.filter(s =>
                              SERVICE_STATION_TYPES.includes(s.stationType)
                            )
                            ?.filter(s => s.distance === 0)?.length === 0 && (
                            <p>
                              <small>
                                <i
                                  className='icon icarus-terminal-warning'
                                  style={{
                                    position: 'relative',
                                    top: '.25rem'
                                  }}
                                />
                                No known {service} in system
                              </small>
                            </p>
                          )}
                          {nearestServices[service]
                            ...
                            <Collapsible
                              ...
                              trigger={
                                <p
                                  className='muted text-uppercase'
                                  style={{
                                    display: 'block',
                                    margin: '.5rem 0 0 0'
                                  }}
                            ...
                            >
                              {nearestServices[service]
                                ...
                                ?.map(station => (
                                  <Fragment
                                    key={`in_system_service_${service}_${station.marketId}`}
                                  >
                                    <p
                                      style={{
                          ...
                          {nearestServices[service]
                            ...
                            >
                              {nearestServices[service]
                                ...
                                ?.map(station => (
                                  <Fragment
                                    key={`nearest_service_${service}_${station.marketId}`}
                                  >
                                    <p
                                      style={{
                                        padding: '.25rem .25rem 0 .25rem',
                                    ...
                                    >
                                      <StationIcon station={station}>
                                        ...
                                        <small className='text-no-transform'>
                                          {' '}
                                          {station.distance.toLocaleString()} ly
                                        </small>
                                        <br />
                                        {station.updatedAt && (
                                          <small>
                          ...
                          {nearestServices[service]?.filter(
                            ...
                            >
                              {nearestServices[service]
                                ?.filter(s => s.stationType === 'FleetCarrier')
                                ?.sort((a, b) =>
