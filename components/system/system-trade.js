...
const SystemTrade = ({
  ...
  const activeTab = router.query?.slug?.[1] === 'imports' ? 1 : 0
            ...
            <Table
              ...
              columns={[
                    ...
                    <>
                      ...
                      <div className='is-visible-mobile'>
                        <small style={{ float: 'right' }}>
                          {r.exportOrders.length === 1
                            ? '1 exporter '
                            : `${r.exportOrders.length} exporters`}
                        </small>
                      </div>
                      <small>{r.category}</small>
                      {r?.rare === true && (
                        <small style={{ opacity: 1 }} className='text-rare'>
                          {' '}
                          Rare
                        </small>
                      )}
                          ...
                          <tbody style={{ textTransform: 'uppercase' }}>
                            <tr>
                              <td>
                                ...
                                {r.totalStock.toLocaleString()} T
                              </td>
                              <td className='text-right'>
                                <span className='data-table__label'>Price</span>
                                {r.avgPrice.toLocaleString()} CR
                                <br />
                                {r.galacticAvgPrice > 0 &&
                                  r.avgPrice > r.galacticAvgPrice && (
                ...
                },
                {
                  ...
                  render: v => (
                    <span className='counter counter--table-counter'>
                      {v.length === 1 ? '1' : `${v.length}`}
                    </span>
                  )
                },
                {
                    ...
                    <>
                      ...
                      {r.galacticAvgPrice > 0 &&
                        r.avgPrice > r.galacticAvgPrice && (
                          <small className='commodity__price text-negative'>
                            -
                            {(r.avgPrice - r.galacticAvgPrice).toLocaleString()}{' '}
                            CR
                          </small>
                        )}
                      {r.avgPrice < r.galacticAvgPrice && (
                        <small className='commodity__price text-positive'>
                          +{(r.galacticAvgPrice - r.avgPrice).toLocaleString()}{' '}
                          CR
                        </small>
                      )}
                    </>
                    ...
                    <p className='table-row-expanded-link'>
                      {r?.rare === true && (
                        <div
                          ...
                          onClick={() => {
                            const rare = rareGoods.filter(
                              rare =>
                                rare.name.toLowerCase() === r.name.toLowerCase()
                            )?.[0]
                            const contents = rare ? (
                              <>
                                <p>
                                  {rare.stationName} in {system.systemName} is
                                  the exclusive exporter of {rare.name}.
                                </p>
                                <p>{rare?.description}</p>
                                <p>
                                  {rare?.limit && (
                                    <>
                                      Export restrictions typically limit orders
                                      to {rare.limit}T at a time.
                                    </>
                                  )}
                                </p>
                              </>
                              ...
                              <p>
                                {r.systemName} in {system.systemName} is the
                                exclusive exporter of {r.name}.
                              </p>
                            )
                      ...
                      {r?.rare !== true && (
                        <Link
                          className='button--small'
                          href={`/commodity/${r.symbol}/exporters?location=${encodeURIComponent(r.systemName)}&maxDistance=25`}
                        >
                      ...
                      <Link
                        className='button--small'
                        href={`/commodity/${r.symbol}/importers?location=${encodeURIComponent(r.systemName)}&maxDistance=25`}
                      >
              ...
              columns={[
                {
                  ...
                  render: (v, r) => (
                    <>
                      ...
                      <div className='is-visible-mobile'>
                        <small style={{ float: 'right' }}>
                          {r.importOrders.length === 1
                            ? '1 importer '
                            : `${r.importOrders.length} importers`}
                        </small>
                      </div>
                      <small>
                        {r.category}
                          ...
                          <tbody style={{ textTransform: 'uppercase' }}>
                            <tr>
                              <td>
                                ...
                                {r.totalDemand > 0 ? (
                                  `${r.totalDemand.toLocaleString()} T`
                                ) : (
