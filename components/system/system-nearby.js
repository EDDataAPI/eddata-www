...
const SystemNearby = ({ nearbySystems }) => {
  const router = useRouter()

  const onSystemsRowClick = (record, _index, _event) => {
  ...
  return (
    <div className='fx__fade-in'>
      ...
      <Table
              ...
              <SystemObjectIcon type='Star'>
                ...
                <small className='is-visible-mobile text-no-transform'>
                  ...
                  {r?.distance < 1 ? (
                    ...
                    <>{Math.floor(r.distance).toLocaleString()} ly</>
                  )}
                </small>
              </SystemObjectIcon>
            )
        ...
        data={nearbySystems}
        emptyText={
          nearbySystems === undefined || nearbySystems === null ? (
            <span className='muted'>Searching...</span>
          ) : (
            <span className='muted'>No nearby systems known</span>
          )
        }
