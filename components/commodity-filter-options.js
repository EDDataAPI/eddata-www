...
function CommodityFilterOptions({
  ...
  const [commodityAutoCompleteResults, setCommodityAutoCompleteResults] =
    useState()
      ...
      >
        <InputWithAutoComplete
          ...
          onChange={e => {
            ...
            commodities.forEach(commodity => {
              if (
                commodity.name.toLowerCase() === commodityName.toLowerCase()
              ) {
                isExactMatch = true
              }
              if (
                commodity.name
                  .toLowerCase()
                  .startsWith(commodityName.toLowerCase())
              ) {
            ...
            commodities.forEach(commodity => {
              if (
                ...
                commodity.name
                  .toLowerCase()
                  .includes(commodityName.toLowerCase())
              ) {
                autoCompleteResults.push({
                  icon: 'cargo',
                  text: commodity.name,
            ...
            if (
              (isExactMatch === true && autoCompleteResults.length === 1) ||
            ...
            if (!text) {
              ...
              if (commodity) {
                window.dispatchEvent(
                  new CustomEvent('LoadCommodityEvent', {
                    detail: commodity.symbol
                  })
                )
              } else {
                commodityRef.current.value = selectedCommodity.name
        ...
        <InputWithAutoComplete
          ...
          onChange={async e => {
            ...
            try {
              const searchText =
                !systemName || systemName === '' ? 'A' : systemName
            ...
            systemSearchResults.forEach(system => {
              if (
                !system.systemName.startsWith(systemName) &&
                system.systemName
                  .toLowerCase()
                  .startsWith(systemName.toLowerCase())
              ) {
                autoCompleteResults.push({
                  icon: 'star',
                  text: system.systemName,
        ...
        >
          {locationRef.current?.dataset?.value ? (
            <span
              onClick={() =>
                router.push(`/system/${locationRef.current?.dataset?.value}`)
              }
            >
              <span className='muted'>SYS ADDR</span>{' '}
              <span
                style={{ borderBottom: '1px dotted var(--color-primary-5)' }}
              >
                {locationRef.current?.dataset?.value}
              </span>
            ...
            <span className='muted' style={{ paddingRight: '1rem' }}>
              ...
            </span>
          )}
        </small>
        <label>
          ...
          <select
            ref={maxDistanceRef}
            name='maxDistance'
            disabled={disabled || !locationRef.current?.dataset?.value}
            onChange={optionChangeHandler}
          >
            <option value={COMMODITY_FILTER_DISTANCE_DEFAULT}>
              Any distance
            </option>
