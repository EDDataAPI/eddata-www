      ...
      <label className='input-with-autocomplete__label'>
        ...
        {focus === true && _autoCompleteResults?.length > 0 && (
          <div className='input-with-autocomplete__results-dropdown'>
            <div
              ref={resultsRef}
              className='input-with-autocomplete__results scrollable'
            >
                ...
                <div key={`autocomplete-${name}-${result.value}-${i}`}>
                  {!result?.seperator && !result.disabled && (
                    ...
                    >
                      {result?.icon && (
                        <i
                          className={`input-with-autocomplete__result-icon icarus-terminal-${result?.icon}`}
                  ...
                  {result?.disabled === true && (
                    <p className='input-with-autocomplete__result input-with-autocomplete__result--disabled muted'>
                      {result?.text}
                    </p>
                  )}
                  {result?.seperator === true && (
                    <hr key={`seperator-${i}`} style={{ margin: '.25rem 0' }} />
                  )}
                </div>
              ))}
            </div>
