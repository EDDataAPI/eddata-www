module.exports = ({ stationType, station, children }) => {
  ...
  if (!_stationType && station?.bodyName) {
    icon = 'settlement'
  }
  ...
  if (
    !_stationType &&
    station?.stationName.match('^[A-Z0-9]{3}-[A-Z0-9]{3}$')
  ) {
    icon = 'fleet-carrier'
  }
  if (!_stationType && station?.stationName === 'Stronghold Carrier') {
    icon = 'megaship'
  }
  ...
  if (_stationType === 'StrongholdCarrier') {
    icon = 'megaship'
  } // Note: This is not an offical FDev type
    ...
    return (
      <span className='system-object'>
        <i
          ...
          className={`system-object-icon icarus-terminal-${icon}`}
        />
        {children}
      </span>
    )
