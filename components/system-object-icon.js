module.exports = ({ systemObject, type, children }) => {
  ...
  if (_systemObjectType === 'Star') {
    ...
    if (
      ...
      !systemObject?.subType?.toLowerCase()?.includes('gas giant')
    ) {
      icon = 'planet-atmosphere'
    }

    if (
      systemObject?.subType?.toLowerCase() === 'high metal content world' ||
    ...
    if (
      systemObject?.subType?.toLowerCase()?.includes('water world') ||
      systemObject?.subType?.toLowerCase()?.includes('water giant')
    ) {
    ...
    return (
      <span className='system-object'>
        <i
          ...
          className={`system-object-icon icarus-terminal-${icon}${systemObject?.isLandable ? ' text-info' : ''}`}
        />
        {children}
      </span>
    )
