...
function CommodityImportersNearby({ commodity, rare }) {
  const [nearbyImporters, setNearbyImporters] = useState()

  useEffect(() => {
    ;(async () => {
      const _nearbyImporters = await getNearbyImportersOfCommodity(
        commodity.systemAddress,
        commodity.symbol
      )
  ...
  return (
    <>
      {!nearbyImporters && (
        <div className='loading-bar' style={{ marginTop: 0 }} />
      )}
      {nearbyImporters && (
        <Table
            ...
            },
            {
              ...
              render: (v, r) => (
                <>
                  ...
                  {r.distance !== undefined && (
                    <small class='no-wrap' style={{ textTransform: 'none' }}>
                      {' '}
                      {r.distance.toLocaleString()} ly
                    </small>
                  )}
          ...
          rowKey={r =>
            `nearby_commodity_importers_${r.marketId}_${r.commodityName}`
          }
        />
      )}
    </>
...
export default CommodityImportersNearby
