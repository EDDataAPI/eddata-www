...
function Header() {
  ...
  async function onSearchInputChange(e) {
    ...
    try {
      const matchingCommodities = listOfCommoditiesAsArray.filter(c =>
    ...
    try {
      ...
      try {
        const res = await fetch(
          `${API_BASE_URL}/v2/search/station/name/${searchText}`
        )
    ...
    try {
      ...
      try {
        const res = await fetch(
          `${API_BASE_URL}/v2/search/system/name/${searchText}`
        )
      ...
      if (searchText?.length > 3 && Number.isInteger(parseInt(searchText))) {
        try {
          const systemByAddressRes = await fetch(
            `${API_BASE_URL}/v2/system/address/${searchText}`
          )
  ...
  useEffect(() => {
    ...
    if (Array.isArray(commoditySearchResults)) {
      commoditySearchResults.forEach(i =>
        searchResults.push({
    ...
    if (Array.isArray(systemSearchResults)) {
      systemSearchResults.forEach(i =>
        searchResults.push({
          icon: 'icarus-terminal-star',
          name: i.systemName,
          description: `${i.systemX}, ${i.systemY}, ${i.systemZ}`,
          path:
            i?.ambiguous !== undefined || i?._useSystemAddress === true
  ...
  useEffect(() => {
    const onKeyDown = ({ key }) => {
      if (key === 'Enter') {
        if (
          document.activeElement?.id === 'headerSearch' ||
    ...
    <header>
      <div className='header__logo'>
        ...
        <ul
          ...
          style={{ position: 'relative', top: '-.5rem', left: '.6rem' }}
        >
      ...
      <div className='header__navigation' style={{ display: 'block' }}>
        <div className='header__search'>
          <label
            className='header__search-input header__search-input--always-visible'
            aria-label='Search'
            onClick={() => headerSearchRef.current.focus()}
          >
            ...
            {searchResults?.length > 0 &&
              searchResults.map((result, i) => (
                <p
                  ...
                  className={
                    i === highlightedResult
                      ? 'header__search-result--highlighted'
                      : undefined
                  }
        ...
        >
          <i
            className={`icon ${fullScreenState === true ? 'icarus-terminal-fullscreen-exit' : 'icarus-terminal-fullscreen'}`}
          />
        </button>
        <button
          id='header-menu-toggle'
          aria-label='Menu'
          className={`button header__menu-button is-visible-mobile ${menuVisible ? '--active' : ''}`}
          onClick={() => toggleMenu()}
        >
          <i
            className={`icon icarus-terminal-chevron-${menuVisible ? 'up' : 'down'}`}
          />
        </button>
        <div
          className='header__menu is-visible-mobile'
        ...
        >
          ...
          <button
            aria-label='Toggle Fullscreen'
            className='--no-hover header__menu-item'
            onClick={() => toggleFullScreen()}
          >
            <i
              className={`icon ${fullScreenState === true ? 'icarus-terminal-fullscreen-exit' : 'icarus-terminal-fullscreen'}`}
      ...
      <div className='news-ticker'>
        <div className='news-ticker__clock'>
          {dateTime !== undefined && (
            <div className='fx__fade-in'>
              ...
              <div className='news-ticker__clock-date'>
        ...
        {[...Array(2)].map((arr, i) => (
          <span
            key={`news-ticker-${i}`}
            className={`news-ticker__ticker news-ticker__ticker--${i}`}
          >
            {newsTicker.map(item => (
              <span
                key={`ticker_${i}_${item.marketId}_${item.commodityName}`}
                className='news-ticker__ticker-item'
                onClick={() =>
                  router.push(
                    `/commodity/${item.commodityName}/${item.demandBracket === 3 ? 'importers' : 'exporters'}?location=${encodeURIComponent(item.systemAddress)}&maxDistance=1`
                  )
                }
              >
                <i
                ...
                <span style={{ color: 'var(--color-primary--lighter)' }}>
                  {commodities?.[item.commodityName]?.name ??
                    item.commodityName}
                </span>
                  ...
                  {item.demandBracket > item.stockBracket ? (
                    <>
                      {item.demand === 0 ? (
                        ...
                        <>{item.demand.toLocaleString()} T</>
                      )}
                      <br />
                      {item.sellPrice.toLocaleString()} CR/T
                    </>
                  ) : (
                    <>
                      {item.stock.toLocaleString()} T<br />
                      {item.buyPrice.toLocaleString()} CR/T
                    </>
                  )}
                </span>
                <span className='news-ticker__ticker-item-price-difference'>
                  {item.avgSellPrice !== 0 && item.demandBracket === 3 && (
                    <>
                      {item.sellPrice > item.avgSellPrice && (
                        <small className='muted'>Avg profit</small>
                      )}
                      {item.sellPrice < item.avgSellPrice && (
                        <small className='muted'>Avg Loss</small>
                      )}
                    </>
                  )}
                  {item.avgBuyPrice !== 0 && item.stockBracket === 3 && (
                    <>
                      {item.buyPrice > item.avgBuyPrice && (
                        <small className='muted'>Avg Loss</small>
                      )}
                      {item.buyPrice < item.avgBuyPrice && (
                        <small className='muted'>Avg profit</small>
                  ...
                  {item.demandBracket === 3 && item.avgSellPrice !== 0 && (
                    <>
                      {item.sellPrice > item.avgSellPrice && (
                        <small className='commodity__price text-positive '>
                          +
                          {(
                            item.sellPrice - item.avgSellPrice
                          ).toLocaleString()}{' '}
                      ...
                      {item.sellPrice < item.avgSellPrice && (
                        <small className='commodity__price text-negative'>
                          -
                          {(
                            item.avgSellPrice - item.sellPrice
                          ).toLocaleString()}{' '}
                          CR/T
                        </small>
                      )}
                  ...
                  {item.stockBracket === 3 && item.avgBuyPrice !== 0 && (
                    <>
                      {item.buyPrice > item.avgBuyPrice && (
                        <small className='commodity__price text-negative'>
                          -{(item.buyPrice - item.avgBuyPrice).toLocaleString()}{' '}
                          CR/T
                        </small>
                      )}
                      {item.buyPrice < item.avgBuyPrice && (
                        <small className='commodity__price text-positive'>
                          +{(item.avgBuyPrice - item.buyPrice).toLocaleString()}{' '}
                          CR/T
                        </small>
                      )}
                    </>
