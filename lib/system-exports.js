...
export default async function getSystemExports(systemAddress) {
  const allCommodities = await getCommoditiesWithPricing()
  const res = await fetch(
    `${API_BASE_URL}/v2/system/address/${systemAddress}/commodities/exports?fleetCarriers=0`
  )
  ...
  exportOrders.forEach(c => {
    ...
    if (!exportOrdersGroupedByCommodity[symbol]) {
      exportOrdersGroupedByCommodity[symbol] = {
        ...
        symbol,
        name:
          allCommodities.find(el => el.symbol.toLowerCase() === symbol)?.name ??
          c.commodityName,
        category:
          allCommodities.find(el => el.symbol.toLowerCase() === symbol)
            ?.category ?? 'Unknown',
    ...
    exportOrdersGroupedByCommodity[symbol].avgPrice = Math.round(
      exportOrdersGroupedByCommodity[symbol].totalPrice /
        exportOrdersGroupedByCommodity[symbol].totalStock
    )
    if (
      exportOrdersGroupedByCommodity[symbol].bestPrice === null ||
      c.buyPrice < exportOrdersGroupedByCommodity[symbol].bestPrice
    ) {
      exportOrdersGroupedByCommodity[symbol].bestPrice = c.buyPrice
    }
    if (
      exportOrdersGroupedByCommodity[symbol].updatedAt === null ||
