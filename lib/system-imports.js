...
export default async function getSystemImports(systemAddress) {
  const allCommodities = await getCommoditiesWithPricing()
  const res = await fetch(
    `${API_BASE_URL}/v2/system/address/${systemAddress}/commodities/imports?fleetCarriers=0`
  )
  ...
  importOrders.forEach(c => {
    ...
    const commodityMetadata = allCommodities.find(
      el => el.symbol.toLowerCase() === symbol
    )
    ...
    if (importOrdersGroupedByCommodity[symbol].totalDemand > 0) {
      importOrdersGroupedByCommodity[symbol].totalPrice +=
        c.sellPrice * c.demand
      importOrdersGroupedByCommodity[symbol].avgPrice = Math.round(
        importOrdersGroupedByCommodity[symbol].totalPrice /
          importOrdersGroupedByCommodity[symbol].totalDemand
      )
    } else if (!importOrdersGroupedByCommodity[symbol].avgPrice) {
      importOrdersGroupedByCommodity[symbol].avgPrice = c.sellPrice
    }
    if (
      importOrdersGroupedByCommodity[symbol].bestPrice === null ||
      c.sellPrice > importOrdersGroupedByCommodity[symbol].bestPrice
    ) {
      importOrdersGroupedByCommodity[symbol].bestPrice = c.sellPrice
    }
    if (
      importOrdersGroupedByCommodity[symbol].updatedAt === null ||
  ...
  // Filter out zero demand commodities unless they are marked as consumer
  // Future enhancement: Add UI toggle to show/hide zero demand commodities
  return Object.values(importOrdersGroupedByCommodity)
    .filter(c => c?.totalDemand > 0 || c.consumer === true)
