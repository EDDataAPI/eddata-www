...
function parseBodiesAndStations(bodies = [], stations = [], system) {
  ...
  const _allBodies =
    JSON.parse(JSON.stringify(bodies))?.sort(
      (a, b) => a?.distanceToArrival - b?.distanceToArrival
    ) ?? []
  const _bodies = []
  ...
  for (const body of _allBodies) {
    ...
    if (system?.systemName) {
      body.bodyName = body.bodyName.replace(
        new RegExp(`^${system.systemName}`, 'i'),
        system.systemName
      )
    }
    ...
    if (
      (body.id64 && _bodies.filter(b => b.id64 === body?.id64) < 1) ||
      !body?.id64
    ) {
      _bodies.push(body)
    }
  ...
  if (_bodies.length === 1) {
    ...
    for (const station of _stations) {
      ...
      // to contintue.
      if (station?.bodyName || station?.bodyId) {
        continue
      }
      ...
      if (bodyDistances.length > 1) {
        ...
        const closestBodyInDistance = bodyDistances.reduce(
          function (prev, curr) {
            const stationDistance = station?.distanceToArrival ?? 0
            return Math.abs(curr - stationDistance) <
              Math.abs(prev - stationDistance)
              ? curr
              : prev
          }
        )
        ...
        for (const body of _bodies) {
          if (closestBodyInDistance === body.distanceToArrival) {
            ...
            if (
              planetaryStations.includes(station?.stationType) &&
              !body.isLandable
            ) {
              continue
            }
      ...
      if (
        ...
        const closestBodyInDistance = _bodies
          .map(body => body.distanceToArrival)
  ...
  _stations.forEach(station => {
    if (station?.bodyName === null && station?.bodyId === null) {
      ...
      _bodies.forEach(body => {
        if (station?.bodyName === body?.bodyName) {
          validLocationFound = true
        }
        if (station?.bodyId === body?.bodyId) {
          validLocationFound = true
        }
      })
  ...
  if (stationsWithNoKnownLocation.length > 0) {
  ...
  return _bodies
    .map(body => {
      ...
      if (
        ...
        (!body?.bodyId || body?.bodyId === body.id)
      ) {
        body.bodyId = 0
      }

      const nearbyStations = _stations.filter(
        station =>
      ...
      body.stations = body.stations
        ? body.stations.concat(nearbyStations)
        : nearbyStations

      // Get all other bodies directly orbiting this body
      const directChildren = _bodies.filter(
        b => b?.parents?.[0]?.[body?.bodyType] === body?.bodyId
      )
      ...
      const indirectChildren = _bodies.filter(child => {
        ...
        const isOrbitingDirectly =
          child?.parents?.[0]?.[body?.bodyType] === body?.bodyId
        if (isOrbitingDirectly) {
          return false
        }
        ...
        for (const parent of child?.parents ?? []) {
          ...
          if (parent[body?.bodyType] === body?.bodyId) {
            return true
          }
          if (parent[body?.bodyType] !== body?.bodyId) {
            return false
