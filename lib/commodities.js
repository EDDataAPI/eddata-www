...
const listOfCommoditiesAsArray = Object.keys(listOfCommodities).map(
  c => listOfCommodities[c]
)

async function getCommodityBySymbol(symbol) {
  const commoditiesWithPricing = await getCommoditiesWithPricing()
  return commoditiesWithPricing?.filter(
...
async function getCommoditiesWithPricing(commoditiesWithPricingInput = null) {
  ...
  let commoditiesWithPricing =
    commoditiesWithPricingInput || COMMODITY_PRICE_DATA_CACHED_RESPONSE
  if (commoditiesWithPricing === null) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        console.warn(`Failed to fetch commodities: ${res.status}`)
        commoditiesWithPricing = []
      }
    } catch (e) {
  ...
  // Ensure commoditiesWithPricing is an array
  if (!Array.isArray(commoditiesWithPricing)) {
    commoditiesWithPricing = []
  }
  ...
  return listOfCommoditiesAsArray
    .map(commodityWithoutPricing => {
      const commodityWithPricing = commoditiesWithPricing.find(
        el =>
          commodityWithoutPricing.symbol.toLowerCase() ===
          el.commodityName.toLowerCase()
      )
      const commodity = commodityWithPricing
        ? { ...commodityWithoutPricing, ...commodityWithPricing }
        : commodityWithoutPricing
      ...
      commodity.avgProfitMargin = Math.floor(
        (commodity.avgProfit / commodity.avgBuyPrice) * 100
      )
      commodity.maxProfit = commodity.maxSellPrice - commodity.minBuyPrice
      if (!commodity.totalDemand) {
        commodity.totalDemand = 0
      }
      if (!commodity.avgSellPrice) {
        commodity.avgSellPrice = 0
      }
      if (!commodity.totalStock) {
        commodity.totalStock = 0
      }
      if (!commodity.avgBuyPrice) {
        commodity.avgBuyPrice = 0
      }
      return commodity
    })
