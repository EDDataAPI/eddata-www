...
export default function Page(props) {
  ...
  useEffect(() => {
    ;(async () => {
      ...
      const filterByCategory = router.query.slug
        ? (commodities_.filter(
            c => c.category.toLowerCase() === router.query.slug[0].toLowerCase()
          )?.[0]?.category ?? false)
        : false
  ...
  return (
    <Layout
      ...
      title={
        categories?.length === 1
          ? `${categories.filter(category => category.toLowerCase() !== 'nonmarketable').map(category => category)} commodities - Elite Dangerous`
          : 'Commodities in Elite Dangerous'
      }
      description={
        categories?.length === 1
          ? `Where to buy and sell ${categories.filter(category => category.toLowerCase() !== 'nonmarketable').map(category => category)} in Elite Dangerous`
          : 'Where to buy and sell commodities in Elite Dangerous'
      }
    >
      <Head>
        ...
        <div className='fx__fade-in'>
          ...
          {categories?.length > 1 && (
            <div className='menu-button-grid' style={{ marginTop: '.5rem' }}>
              {categories
                .filter(category => category.toLowerCase() !== 'nonmarketable')
                .map(category => (
                  <Link
                    key={`category-button__${category}`}
                    className='button'
                    href={`/commodities/${category.toLowerCase()}`}
                  >
                    {category}
                  </Link>
                ))}
                ...
                <div key={`category_${category}`} style={{ marginTop: '1rem' }}>
                  {categories?.length > 1 && (
                    <h2
                      ...
                      onClick={() =>
                        router.push(`/commodities/${category.toLowerCase()}`)
                      }
                    >
                      {category}
                    </h2>
                                ...
                                <tbody style={{ textTransform: 'uppercase' }}>
                                  <tr>
                                    <td>
                                      ...
                                      {r.avgBuyPrice > 0 ? (
                                        <>{r.avgBuyPrice.toLocaleString()} CR</>
                                      ) : (
                                        '-'
                                      )}
                                    </td>
                                  ...
                                  <tr>
                                    <td>
                                      ...
                                      {r.avgSellPrice > 0 ? (
                                        <>
                                          {r.avgSellPrice.toLocaleString()} CR
                                        </>
                                      ) : (
                                        '-'
                                      )}
                      ...
                      },
                      {
                        ...
                        render: (v, r) => (
                          <div style={{ textTransform: 'uppercase' }}>
                            ...
                            {v > 0 ? (
                              <small>
                                Max {r.maxProfit.toLocaleString()} CR
                              </small>
                            ) : (
                              ''
                            )}
