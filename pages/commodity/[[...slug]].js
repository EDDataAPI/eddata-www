...
export default () => {
  ...
  async function loadCommodity(_commoditySymbol, _activeTab) {
    ...
    if (c) {
      if (commoditySymbol.toLowerCase() !== c.symbol.toLowerCase()) {
        router.push(
          `/commodity/${c.symbol.toLowerCase()}/${_activeTab}${window.location.search}`
        )
        return
      }
      setCommodity(c)
  ...
  return (
    <Layout
      ...
      description={
        commodity
          ? `Where to buy and sell ${commodity.name} in Elite Dangerous`
          : null
      }
      sidebar={
        commodity !== null ? (
      ...
      {commodity && (
        <div className='sticky-heading fx__fade-in'>
          <Tabs
            ...
            onSelect={index => {
              router.push(
                `/commodity/${commodity?.symbol.toLowerCase()}/${TABS[index]}${window.location.search}`
              )
            }}
          >
            <TabList>
              <Tab>
                <i
                  ...
                  className='icarus-terminal-cargo-export'
                />
              ...
              <Tab>
                <i
                  ...
                  className='icarus-terminal-cargo-import'
                />
            ...
            <TabPanel>
              ...
              {exports && (
                <CommodityExporters
                  tableName={`${commodity.name} Exporters`}
                  commodities={exports}
                />
              )}
            </TabPanel>
            <TabPanel>
              ...
              {imports && (
                <CommodityImporters
                  tableName={`${commodity.name} Importers`}
                  commodities={imports}
                  rare={!!rareMarket}
                />
              )}
...
function apiQueryOptions() {
  ...
  const lastUpdatedFilterValue =
    query?.maxDaysAgo ?? COMMODITY_FILTER_MAX_DAYS_AGO_DEFAULT
  const minVolumeFilterValue =
    query?.minVolume ?? COMMODITY_FILTER_MIN_VOLUME_DEFAULT
  const fleetCarrierFilterValue =
    query?.fleetCarriers ?? COMMODITY_FILTER_FLEET_CARRIER_DEFAULT
  const locationFilterValue =
    query?.location ?? COMMODITY_FILTER_LOCATION_DEFAULT
  const distanceFilterValue =
    query?.maxDistance ?? COMMODITY_FILTER_DISTANCE_DEFAULT
  ...
  if (
    ...
    if (
      distanceFilterValue &&
      distanceFilterValue !== COMMODITY_FILTER_DISTANCE_DEFAULT
    ) {
    ...
    <div style={{ paddingTop: '.5rem' }}>
      ...
      <div className='fx__fade-in' style={{ padding: '0 .1rem' }}>
        <span
          ...
          style={{ lineHeight: '1.25rem', display: 'block', marginTop: '1rem' }}
        >
          {commodity.name}
        </span>

        <p
          className='fx__animated-text text-uppercase'
          data-fx-order='3'
          style={{ marginBottom: '.25rem', fontSize: '.9rem' }}
        >
          <Link href={`/commodities/${commodity.category.toLowerCase()}`}>
            {commodity.category.replace(/s$/, '')}
          </Link>
        </p>
        <p
          className='text-no-transform muted'
        ...
        {commodity?.rare &&
          rareMarket?.stationName &&
          rareMarket?.systemName && (
            <>
              <span
                className='text-rare text-uppercase'
                style={{
                  fontSize: '.9rem',
                  lineHeight: '1.5rem',
                  display: 'block'
              ...
              <StationIcon station={rareMarket}>
                ...
                >
                  <Link
                    href={`/commodity/${commodity.symbol.toLowerCase()}?location=${rareMarket.systemName}`}
                  >
                    {rareMarket.stationName}
                  </Link>
                ...
                <div
                  ...
                  onClick={() => {
                    const contents = commodity?.description ? (
                      <>
                        <p>
                          {rareMarket.stationName} in {rareMarket.systemName} is
                          the exclusive exporter of {commodity.name}.
                        </p>
                        <p>{commodity?.description}</p>
                        <p>
                          {commodity?.limit && (
                            <>
                              Export restrictions typically limit orders to{' '}
                              {commodity.limit}T at a time.
                            </>
                          )}
                        </p>
                      </>
                      ...
                      <p>
                        {rareMarket.systemName} in {rareMarket.systemName} is
                        the exclusive exporter of {commodity.name}.
                      </p>
                    )
