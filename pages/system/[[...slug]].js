...
function System() {
  ...
  useEffect(() => {
    ;(async () => {
      ...
      setNavigationPath([
        { name: 'Systems', path: '/', icon: 'icarus-terminal-system-orbits' }
      ])
      setLoading(true)
      ...
      if (_system) {
        ...
        ;(async () => {
          ...
          for (const [, commodity] of Object.entries(listOfCommodities)) {
            if (
              marketIds.includes(parseInt(commodity.market_id)) &&
              commodity.rare
            ) {
              rareItems.push({
                stationName: stations.filter(
                  s => s.marketId === parseInt(commodity.market_id)
                )[0].stationName,
                ...commodity
              })
            }
        ...
        ;(async () => {
          ...
          setNearbySystems(
            nearbySystems.filter(
              s => !HIDDEN_SYSTEMS.includes(`${s.systemAddress}`)
            )
          )
        })()
        ;(async () => {
          ...
          importOrders.forEach((order, i) => {
            if (
              ...
              new Date(mostRecentUpdatedAt).getTime()
            ) {
  ...
  const systemIdentifer = router.query.slug
    ? router.query.slug[0]?.replaceAll('_', ' ')?.trim()
    : null
  const systemViewBaseUrl =
    systemIdentifer && Number.isInteger(parseInt(systemIdentifer))
  ...
  return (
    <Layout
      ...
      description={
        system
          ? `Data for the system ${system.systemName} in Elite Dangerous`
          : null
      }
      navigation={[
        {
        ...
        },
        {
          ...
          active:
            views[activeViewIndex] === 'exports' ||
            views[activeViewIndex] === 'imports'
        },
        {
          name: 'Services',
...
async function getSystem(systemIdentifer) {
  try {
    const systemIdentiferType = systemIdentiferIsSystemAddress(systemIdentifer)
      ? 'address'
      : 'name'
    const res = await fetch(
      `${API_BASE_URL}/v2/system/${systemIdentiferType}/${systemIdentifer}`
    )
...
async function getStationsInSystem(systemIdentifer) {
  try {
    const systemIdentiferType = systemIdentiferIsSystemAddress(systemIdentifer)
      ...
      : 'name'
    const res = await fetch(
      `${API_BASE_URL}/v2/system/${systemIdentiferType}/${systemIdentifer}/stations`
    )
    return res.status === 200 ? await res.json() : null
  } catch (e) {
    console.error(e)
...
async function getNearbySystems(systemIdentifer) {
  try {
    const systemIdentiferType = systemIdentiferIsSystemAddress(systemIdentifer)
      ...
      : 'name'
    const res = await fetch(
      `${API_BASE_URL}/v2/system/${systemIdentiferType}/${systemIdentifer}/nearby?maxDistance=25`
    )
...
async function getBodiesInSystem(systemIdentifer) {
  try {
    const systemIdentiferType = systemIdentiferIsSystemAddress(systemIdentifer)
      ...
      : 'name'
    const res = await fetch(
      `${API_BASE_URL}/v2/system/${systemIdentiferType}/${systemIdentifer}/bodies`
    )
    return await res.json()
  } catch (e) {
    console.error(e)
...
async function getSystemStatus(systemIdentifer) {
  try {
    const systemIdentiferType = systemIdentiferIsSystemAddress(systemIdentifer)
      ...
      : 'name'
    const res = await fetch(
      `${API_BASE_URL}/v2/system/${systemIdentiferType}/${systemIdentifer}/status`
    )
    return await res.json()
  } catch (e) {
    console.error(e)
